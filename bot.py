...
def generate_thread_ollama(google_data, x_data):
    ...
    # Prompt is now fully in English to keep the code clean
    prompt = f"""
    Role: Intellectual editor for '@olanlarsakami' bot.
    Task: Create a 'Digital Detox' news thread in TURKISH.
    ...
    Constraint Rules:
    - NO POLITICS: Exclude any political figures, parties, or debates.
    - NO TRASH: Exclude TV series, magazine gossip, or reality shows.
    - FOCUS: Technology, science, space, economy (non-political), and global lifestyle.
    
    Output Format (All tweets must be in TURKISH):
